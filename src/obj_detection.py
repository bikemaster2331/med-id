...
def is_stable_detection(current_box, history, iou_threshold=0.6):
...
def apply_nms_custom(boxes, iou_threshold=0.5):
    ...
    while len(boxes) > 0:
        ...
        boxes = [box for box in boxes[1:] 
                if calculate_iou(keep[-1][:4], box[:4]) < iou_threshold]
